%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ** Description **
% csfec function implements the complex single frequency estimation and 
% cancellation (CSFEC) algorithm (version for complex input signal) [1,2,3].
%
% ** Syntax**
% [f, A, bin] = csfec(params)
%
% ** Input arguments**
...
[~, params.RMinIdx] = min(abs(params.v0 - params.f_min));
if params.f_max == 0    % if no upperbound is set 
    params.f_max = params.v0(end);
end
[~, params.RMaxIdx] = min(abs(params.v0 - params.f_max));
...
% Initalize variables
f = zeros(params.K, 1);
% f_nnorm = zeros(params.K, 1);
A = zeros(params.K, 1);
alph = zeros(params.K, 1);
...
for qq = 1 : params.NCFC
    ...
    for k = 1 : params.K
        if qq == 1
            ...
            [A(k), alph_i] = max(sigt(1,RMinIdx:RMaxIdx));
            alph(k) = alph_i + RMinIdx - 1;
            f(k) = params.V0(alph(k));
            bin(k) = alph(k);
        end
        ...
        if k > 1
            ...
            for jj = ord
                ...
                for it = 1:5
                    ...
                    b = real(conj(A(jj))*X2a);
                    c = -imag(conj(A(jj))*X1a);
                    Delta = -c/b;
                    F = F + (Delta/(2*pi));
                    fdiff = f(idx)-F;
                    to_lim = abs(fdiff)<eps;
        ...
        idx2 = [1:k];
        q = exp(1j * 2 * pi * (f(idx2) - params.V0));
        to_lim = abs(f(idx2)-params.V0)<eps;
...
function [resh_input] = csfec_checkinput(input)
    ...
    if isreal(resh_input.x0)
        disp('params.x0 should be complex (double), got other type instead')
        disp('Press enter to go on')
        pause
    end
